...
raise NotImplementedError("This test is still being converted to use the odrive_can package")
# `test_runner` is complicated, and doesn't seem to be well documented
# From what I can tell, there is a lot of "black magic" going on yet
...
from odrive_can.enums import GpioMode, AxisError, ControlMode
...
class TestSimpleCAN():
    ...
    def run_test(self, odrive: ODriveComponent, canbus: CanInterfaceComponent, node_id: int, extended_id: bool, logger: Logger):
        ...
        if odrive.yaml['board-version'].startswith("v3."):
            odrive.handle.config.gpio15_mode = GpioMode.CAN_A
            odrive.handle.config.gpio16_mode = GpioMode.CAN_A
        ...
        extended_id = not extended_id
        fence()
        test_assert_eq(axis.error, AxisError.NONE)
        ...
        my_cmd('estop')
        fence()
        test_assert_eq(axis.error, AxisError.ESTOP_REQUESTED)
        ...
        test_assert_eq(axis.error, AxisError.ESTOP_REQUESTED | AxisError.INVALID_STATE)
        ...
        test_assert_eq([msg['error'] for msg in heartbeats], [AxisError.WATCHDOG_TIMER_EXPIRED] * len(heartbeats))
